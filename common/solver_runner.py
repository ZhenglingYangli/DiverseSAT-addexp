...
def solver_command(solver: str, solver_bin: str, input_path: Path, args: argparse.Namespace) -> list[str]:
    if solver == "CPLEX":
        return [sys.executable, str(ROOT / "cplex/solvers/cplex_diversesat.py"), str(input_path), str(args.k), args.encoding, args.se_mode]
    if solver == "CaDiCaL":
        return [sys.executable, str(ROOT / "cadical/solvers/cadical_enumerate.py"), str(input_path), str(args.k)]
    if solver == "CaDiCaL-Greedy":
        return [sys.executable, str(ROOT / "cadical_greedy/solvers/cadical_greedy.py"), str(input_path), str(args.k)]
