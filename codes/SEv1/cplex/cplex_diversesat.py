...
def main() -> None:
    ...
    code = prob.solution.get_status()
    status = prob.solution.status[code]
    # Proven-optimal CPLEX status codes (version-independent):
    #   1   CPX_STAT_OPTIMAL      (LP/QP continuous)
    #   101 CPXMIP_OPTIMAL        (MIP/MIQP exact optimum)
    #   102 CPXMIP_OPTIMAL_TOL    (MIP/MIQP optimum proven within gap tolerance)
    OPTIMAL_CODES = {1, 101, 102}
    feasible = prob.solution.is_primal_feasible()
    opt = int(round(prob.solution.get_objective_value())) if feasible else ""
    if code in OPTIMAL_CODES:
        print("@@@ optimal")
    else:
        print(f"@@@ {status}")
    print(
