...
This matches the historical CaDiCaL-greedy baseline: choose one SAT model, then
iteratively choose a model maximizing distance to the models selected so far.
...
def main() -> None:
    ...
    start = time.time()
    base_cnf = CNF(from_file=args.input_path)
    read_end = time.time()
    models: list[list[int]] = []

    with Solver(name=args.solver_name, bootstrap_with=base_cnf.clauses) as solver:
        if solver.solve():
            first = solver.get_model()[: base_cnf.nv]
            models.append([1 if lit > 0 else -1 for lit in first])
    ...
    while models and len(models) < args.k and time.time() - start < args.cutoff:
        ...
        if model is None:
            break
        trimmed = [1 if lit > 0 else -1 for lit in model[: base_cnf.nv]]
