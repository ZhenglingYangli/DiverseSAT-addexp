#!/usr/bin/env bash
#SBATCH --job-name=ne-WMaxCDCL-SEv3-10-BIN
...
SOLVER_BIN="${WMAXCDCL_BIN:-$ROOT/solvers/MaxSAT/wmaxcdcl}"
python3 "$ROOT/wmaxcdcl/jobs/run_solver.py" \
  --solver WMaxCDCL \
  --solver-bin "$SOLVER_BIN" \
  --input-dir "$ROOT/transform/results/k_10-BIN-SEv3" \
  --instance-list "$INSTANCE_LIST" \
  --out-dir "$ROOT/wmaxcdcl/results/WMaxCDCL-BIN-k10-SEv3" \
  --suffix .wcnf \
  --timeout 7200
